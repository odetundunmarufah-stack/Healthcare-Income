...
const getScore = (answers) => {
  ...
  // Boost if they mention specific prior experience or clear intent in free text
...
const getArchetype = (answers) => {
  ...
  if (freeText.includes("medva") || freeText.includes("medical va") || freeText.includes("virtual assistant") || freeText.includes("medical virtual")) return {
    ...
    description: `Your answers point clearly toward remote and international income. Medical Virtual Assistant roles are actively hiring Nigerian ${specialty} professionals right now — no relocation, dollar pay, and your clinical background is the entry ticket. You are closer to your first USD income than you think.`
  };
  if (freeText.includes("medical writ") || freeText.includes("med writing") || freeText.includes("clinical writ") || freeText.includes("pharma writ")) return {
    ...
    description: `Medical writing is one of the most overlooked remote income paths for Nigerian ${specialty} professionals. Your specialty gives you an edge that no English graduate can replicate. No camera, no social media, no audience needed. Just your knowledge and the right brief.`
  };
  if (freeText.includes("aesthet") || freeText.includes("botox") || freeText.includes("filler") || freeText.includes("skincare clinic")) return {
    ...
    description: `Medical aesthetics is one of the fastest-growing extensions of clinical practice in Nigeria — and your ${specialty} training puts you ahead of the competition before you even start. Your blueprint maps the exact certifications, setup costs, and first client strategy.`
  };
  if (freeText.includes("relocat") || freeText.includes("japa") || freeText.includes("abroad") || incomeType.includes("Remote") || incomeType.includes("dollar")) return {
    name: "The Global Clinical Asset",
    tagline: "Your expertise has a market far beyond Nigeria — and you have not tapped it yet.",
    description: `Your profile points strongly toward remote and international income. There are dollar-denominated roles built specifically for ${specialty} professionals that most Nigerian healthcare professionals have never heard of. You are closer to your first USD income than you think.`
  };
  if (energy.includes("teacher") || content.includes("courses") || content.includes("Writing")) return {
    ...
    description: `You have the rare combination of ${specialty} authority and the ability to communicate it clearly. Nigerian patients, students, and professionals are actively searching for what you know — but they cannot find it in a form they can access and afford. That gap is your business.`
  };
  ...
  if (skills.includes("consulting") || skills.includes("Sales")) return {
    ...
    description: `Your combination of ${specialty} knowledge and strategic thinking puts you in a rare category. Healthcare organisations, startups, and corporates need people who understand both medicine and business. Your profile shows several high-ticket paths that leverage exactly this.`
  };
  return {
    name: "The Underutilised Specialist",
    tagline: "Your clinical depth is your greatest untapped asset.",
    description: `You have built ${specialty} expertise that most people spend years trying to acquire. The challenge is not your capability — it is that nobody has shown you the specific paths that translate your particular combination of skills and personality into income. That changes now.`
  };
};
...
const getBlocker = (answers) => {
  ...
  if (blocker.includes("medva") || blocker.includes("medical va") || blocker.includes("virtual assistant") || blocker.includes("client")) return "You already know the direction. The gap is not awareness — it is the specific knowledge of how to land your first client, what platforms to use, and how to position yourself for the exact roles that pay. Your blueprint closes that gap directly.";
  ...
  if (blocker.includes("start") || blocker.includes("begin") || blocker.includes("overwhelm") || blocker.includes("where")) return "The paralysis of not knowing where to start is the most common barrier we see — and it has nothing to do with capability. Your blueprint gives you one starting point, one first offer, and one 30-day plan. Nothing more than that.";
  if (blocker.includes("tried") || blocker.includes("before") || blocker.includes("stopped") || blocker.includes("failed")) return "You have started before and lost momentum. That is not a character flaw — it is a system problem. Your blueprint is designed around focused sprints and milestone-based progress, not daily habits that depend on willpower.";
  if (blocker.includes("relocat") || blocker.includes("japa") || blocker.includes("abroad")) return "Planning to relocate actually makes your income path more urgent, not less. Building a remote income stream before you leave is one of the smartest moves you can make right now. Your blueprint specifically maps paths that work whether you are in Nigeria or abroad.";
  ...
  return `Something specific is standing between you and the income your ${specialty} background should be generating. Your blueprint names it clearly, addresses it practically, and gives you a route around it — not through motivation, but through a plan built for your actual situation.`;
};
